...
begin
	...
	declare
		...
		begin
			Web.Header (Output, Web.HTML, Cookie, Now + Cookie_Duration, Web.Request_URI);
			...
			begin
				...
				case User_State is
					when Users.Managing.Log_Off =>
						Web.Header (Output, Web.HTML, Cookie, Now + Cookie_Duration);
						Web.Header_Break (Output);
...
exception
	when Web.Lock_Files.Lock_Error =>
		...
		Web.Header_Break(Output);
		String'Write(Output, "White fog. Wait 1 minute!" & Web.Line_Break);
	when E : others =>
		Web.Header(Output, Web.Text);
		Web.Header_Break(Output);
