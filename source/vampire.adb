							...
							begin
								...
								if User_State /= Users.Managing.Valid then
									...
									if Village.Time = Villages.Night then
										...
										declare
											Target : constant Integer := Integer'Value(Ase.Web.Element(Inputs, "target"));
											Apply : constant Boolean := Ase.Web.Checkbox_Value(Ase.Web.Element(Inputs, "apply"));
										begin
											...
											elsif Target >= 0 and then not Village.People(Target).Records(Village.Today).Candidate then
												Ase.Web.Header(Output, Ase.Web.HTML, Cookie, Now + Cookie_Duration);
												Ase.Web.Header_Break(Output);
												Renderer.Message_Page(Output, Village_Id, Village'Access, "仮投票が行われました。選ばれた候補以外に投票はできなくなります。", User_Id, User_Password);
											else
												if Target /= Village.People(Player).Records(Village.Today).Vote
													or else Apply /= Village.People(Player).Records(Village.Today).Applied
												then
